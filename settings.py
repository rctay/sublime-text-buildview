...
class SettingsDeclaration(object):
    ...
    def kls_get_value(kls, settings=None):
        ...
        lookup = settings.get(kls.namespace, {})
        value = lookup.get(kls.key, kls.default)
        return value

    @classmethod
    def kls_set_value(kls, settings, value):
        if not settings.has(kls.namespace):
            d = {kls.key: value}
        else:
            d = settings.get(kls.namespace)
            d[kls.key] = value
        settings.set(kls.namespace, d)
